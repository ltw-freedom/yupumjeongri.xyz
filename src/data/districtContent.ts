...
export function dongLead(city: DistrictCity, dong: DistrictDong | DongAlias): string {
  ...
  const variants = [
    // 0 비용·견적
    `${display} 유품정리 비용을 먼저 공개합니다. 원룸 40만원대(부가세 별도)부터 주거 형태별 기준 범위 안에서 견적을 내고, 견적서에 없는 금액은 청구하지 않습니다.`,
    // 1 절차·일정
...
export function dongCostNote(city: DistrictCity, dong: DistrictDong | DongAlias): string {
  ...
  const variants = [
    ...
    `아래 기준은 ${display}을 포함한 서비스 전 지역에 동일하게 적용됩니다. 주거 형태와 짐의 양만 알려주시면 방문 전에 이 범위 안에서 예상 견적을 안내드립니다.`,
  ...
  const bySkeleton = [
    {
      ...
      a: `원룸 기준 40만~90만원, 20평대 아파트 기준 150만~250만원 범위입니다(부가세 별도). ${short} 지역이라고 달라지는 금액은 없고, 물품 양과 반출 동선에 따라 범위 안에서 정해집니다.`,
    },
    {
    ...
    },
    {
      ...
      a: `가능합니다. 유품정리에 특수청소·소독을 더하면 오염 범위에 따라 80만~200만원(부가세 별도)이 추가되며, 방문 확인 때 항목별로 나눠 안내드립니다.`,
    },
    {
      q: `${display}의 오래 비어 있던 집도 정리되나요?`,
      a: `가능합니다. 빈집 정리는 평형보다 쌓인 폐기물 양이 견적 기준이라, 방이 몇 개이고 짐이 어느 정도 쌓였는지만 알려주셔도 방문 전에 예상 범위를 안내드릴 수 있습니다.`,
    },
  ];
  const second = pick(
    [
      {
        q: `${short} 지역도 방문 견적을 받을 수 있나요?`,
        a: `네, ${display}을 포함한 서울·경기 전 지역에서 방문 견적을 진행합니다. 상담 회신 때 주거 형태와 짐의 양을 여쭤보고 방문 전에 대략적인 범위도 안내드립니다.`,
      },
      {
